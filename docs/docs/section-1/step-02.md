...
In this step, we will play with various configurations of the language model (LLM) that we will use in the subsequent
steps.
...
--8<-- "../../section-1/step-01/src/main/resources/META-INF/microprofile-config.properties"
```
...
!!! important "Reloading"
    After changing a configuration property, you need to force a restart of the application to apply the changes. Simply
    submitting a new chat message in the UI does not trigger it (it only sends a websocket message rather than an HTTP
    request), so you have to refresh the page in your browser.
...
```properties
dev.langchain4j.cdi.plugin.customer-support-agent.config.temperature=0.1
```
...
then set the temperature to`1.5` and ask the question again, observing the different styles of the responses. With a too
...
```properties
dev.langchain4j.cdi.plugin.customer-support-agent.config.max-tokens=20
```

to `src/main/resources/application.properties` and see how the model cuts off the response after 20 tokens.

Tokens are not words, but rather the smallest units of text that the model can generate. For example, "Hello, world!"
has 3 tokens: "Hello", ",", and "world". Each model has a different tokenization scheme, so the number of tokens in a
sentence can vary between models.
...
```properties
dev.langchain4j.cdi.plugin.customer-support-agent.config.frequency-penalty=2
```
...
--8<-- "../../section-1/step-02/src/main/resources/META-INF/microprofile-config.properties"
```
